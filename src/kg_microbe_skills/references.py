...
def _resolve_declared(
    ...
    if declared == ANY_MECH:
        ...
        # Absent from the Mechs that ARE here says nothing about the ones that
        # are not. CI checks out three of five, so calling this missing would
        # fail on a TraitMech path for no reason other than TraitMech not being
        # cloned -- #161's rule, which this branch had skipped.
        unresolvable = sorted(set(mechs) - set(available))
        if unresolvable:
            return Finding(
                reference,
                "unverifiable",
                f"declared per-Mech, absent from {', '.join(sorted(available))}, "
                f"and {', '.join(unresolvable)} could not be checked",
            )
        return Finding(
            reference,
            "missing",
